...
export default function AdminSupportPage() {
  ...
  const statCards = [
    { label: "Open", value: stats?.openTickets ?? 0, icon: Inbox, color: "bg-blue-500" },
    { label: "In Progress", value: stats?.inProgressTickets ?? 0, icon: Clock, color: "bg-yellow-500" },
    { label: "Resolved Today", value: stats?.resolvedToday ?? 0, icon: CheckCircle2, color: "bg-green-500" },
  ];
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Support ticket queue</p>
              </div>
            </div>
          </div>
        </div>
              ...
              {tickets.length === 0 ? (
                ...
                tickets.map((t) => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-mono text-gray-700">{t.ticketNumber}</td>
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{t.subject}</td>
                    <td className="px-6 py-4 text-sm text-gray-700">{t.user?.email || "-"}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(t.priority)}`}>
                        {t.priority}
                      </span>
                    </td>
                    <td className="px-6 py-4">
