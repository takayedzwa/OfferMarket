...
export default function AdminOffersPage() {
  ...
  const employmentTypeLabel = (type: string) => {
    try {
      return tEnums("employmentType." + type);
    } catch {
      return type;
    }
  };
  ...
  const formatSalary = (min?: number, max?: number) => {
    ...
    if (min && max) return t("salaryRange", { min: currency(min, "EUR"), max: currency(max, "EUR") });
    if (min) return t("salaryFrom", { amount: currency(min, "EUR") });
    return t("salaryUpTo", { amount: currency(max || 0, "EUR") });
  };
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{t("offersTotal", { total })}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border shadow-sm p-4 mb-6">
          <div className="flex flex-wrap gap-4">
            ...
            >
              ...
              <option value="FILLED">{statusLabel("FILLED")}</option>
            </select>
            <button
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                        ...
                        {offer.employmentTypes?.length && (
                          <div className="flex items-center gap-1">
                            ...
                            <span>{offer.employmentTypes.map(employmentTypeLabel).join(', ')}</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t">
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    <div className="flex items-center gap-1">
                      ...
                      <span>{t("posted", { date: date(offer.createdAt) })}</span>
                    </div>
                    {offer.expiresAt && (
                      <div className="flex items-center gap-1">
                        <Calendar className="w-4 h-4" />
                        <span>{t("expires", { date: date(offer.expiresAt) })}</span>
                      </div>
                    )}
                  </div>
                  <button
