...
export default function ConversationDetailPage() {
  ...
  const getOtherParty = () => {
    if (!conversation) return { name: t("unknown"), icon: User };

    // participant1 is the worker, participant2 is the employer
    if (userRole === "WORKER") {
      return {
        name: conversation.employer?.companyName || t("fallbackEmployer"),
        icon: Building2,
      };
    }
    return {
            ...
            <div>
              ...
              {conversation.offer && (
                <p className="text-sm text-gray-500">
                  {t("regarding", { jobTitle: conversation.offer.jobTitle })}
                </p>
              )}
            </div>
          </div>
